...
<LCMNT>[^\n]*		{}
<LCMNT>[\n]		{curr_lineno++; BEGIN(0);}
<INITIAL>\(\*		{comment_nest++;printf("comment_nest : %d\n" , comment_nest); BEGIN(BCMNT);}
<BCMNT>{BCMT_OP}	{comment_nest++;}
<BCMNT>{BCMT_CL}	{printf("comment_nest : %d\n" , comment_nest); if (--comment_nest ==0) BEGIN(0);}
<BCMNT>[^(*\n]*		{}
<BCMNT>\n		{curr_lineno++;}
...
<INITIAL>{BCMT_CL}	{cool_yylval.error_msg = "Unmatced *)";
			return ERROR;
			}
...
<INITIAL>(?i:esac)	{cool_yylval.symbol = idtable.add_string(yytext);return CASE;}
